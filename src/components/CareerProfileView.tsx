...
function DeferredInput({ value, onCommit, placeholder = "" }: { value: string; onCommit: (value: string) => void; placeholder?: string }) {
  const [draft, setDraft] = useState(value);
  useEffect(() => setDraft(value), [value]);
  return <input value={draft} onChange={(event) => setDraft(event.target.value)} onBlur={() => draft !== value && onCommit(draft)} placeholder={placeholder} />;
}

function DeferredTextarea({ value, onCommit, rows = 5, placeholder = "" }: { value: string; onCommit: (value: string) => void; rows?: number; placeholder?: string }) {
  const [draft, setDraft] = useState(value);
  useEffect(() => setDraft(value), [value]);
...
export function CareerProfileView({ profile, onChange, notify, onOpenDiscovery }: CareerProfileViewProps) {
  ...
  const reflectionCandidates = useMemo(() => buildResumeReflectionCandidates(profile), [profile]);
  const reflectionCount = profile.resumeReflections?.length || 0;
  ...
  return <div className="profile-v3">
    ...
    {profile.resumeText && <section className="panel resume-reflection-summary">
      <div className="resume-reflection-summary-icon"><Sparkles /></div>
      <div><span className="eyebrow">Resume-led Interest Discovery</span><h2>What parts of your strongest accomplishments did you actually enjoy?</h2><p>Your resume proves capability. Reflecting on the actions behind the result helps ReqRadar distinguish work you want to repeat from work you were simply good at.</p><div className="resume-reflection-summary-stats"><span><b>{reflectionCount}</b> reflected</span><span><b>{reflectionCandidates.length}</b> available</span><span><b>{profile.discoveryPreferences.filter((item) => item.evidence.some((evidence) => evidence.sourceType === "RESUME_REFLECTION")).length}</b> themes informed</span></div></div>
      {onOpenDiscovery && <button className="primary" onClick={onOpenDiscovery}>{reflectionCount ? "Continue reflection" : "Start reflection"} <ChevronRight /></button>}
    </section>}
    ...
    <section className="panel peak-panel">
      ...
      {profile.peakExperiences.length ? <div className="peak-list">{profile.peakExperiences.map((experience, index) => <article key={experience.id} className="peak-card"><div className="peak-number">{index + 1}</div><div className="peak-fields"><DeferredInput value={experience.title} onCommit={(value) => onChange(updatePeak(profile, { ...experience, title: value }))} placeholder="Project or moment" /><DeferredTextarea rows={5} value={experience.description} onCommit={(value) => onChange(updatePeak(profile, { ...experience, description: value }))} placeholder="What problem were you solving? What did you personally do? Who were you working with? What made it satisfying?" /><div className="detected-themes"><span>Detected themes</span>{experience.detectedThemes.length ? experience.detectedThemes.map((theme) => <button key={theme} className={experience.confirmedThemes.includes(theme) ? "active" : ""} onClick={() => onChange(updatePeak(profile, { ...experience, confirmedThemes: experience.confirmedThemes.includes(theme) ? experience.confirmedThemes.filter((item) => item !== theme) : [...experience.confirmedThemes, theme] }))}>{experience.confirmedThemes.includes(theme) ? <Check /> : <Plus />}{theme}</button>) : <small>Add more detail to detect themes.</small>}</div></div><button className="icon-btn danger-ghost" onClick={() => onChange({ ...profile, peakExperiences: profile.peakExperiences.filter((item) => item.id !== experience.id), updatedAt: new Date().toISOString() })}><Trash2 /></button></article>)}</div> : <div className="empty-inline"><Lightbulb /> Add one experience to distinguish energizing work from merely familiar work.</div>}
    </section>
