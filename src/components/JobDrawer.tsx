...
export function JobDrawer({ job, assessment, onUpdate, onDelete, onClose, notify, onOpenDiscovery }: JobDrawerProps) {
  ...
  return <><button className="drawer-backdrop" onClick={onClose} aria-label="Close job details" /><aside className="drawer v2">
    <header><div><span className="eyebrow">{job.jobId || "No Job ID"} · {assessment.fingerprint.primaryGroupLabel}</span><h2>{job.title}</h2><div className="drawer-head-badges"><span className={`recommendation ${recommendationClass(assessment.recommendation)}`}>{recommendationLabel(assessment.recommendation)}</span><span className="rank-score">Rank score {assessment.finalScore}</span><span className={`confidence-chip confidence-${assessment.confidence.toLowerCase()}`}>{assessment.confidence.toLowerCase()} confidence</span><span className={`focus-chip focus-${assessment.focusBucket.toLowerCase().replace(/_/g, "-")}`}>{focusBucketLabel(assessment.focusBucket)}</span></div></div><button className="icon-btn" onClick={onClose}><X /></button></header>
    <nav className="drawer-tabs">{(["OVERVIEW", "CAPABILITY", "DISCOVERY", "NETWORKING", "DETAILS"] as Tab[]).map((item) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{item.charAt(0) + item.slice(1).toLowerCase()}</button>)}</nav>
    <div className="drawer-body">
      {tab === "OVERVIEW" && <>
        <section className="assessment-hero v2"><div className="metric-row"><Metric label="Capability" value={assessment.capabilityScore} detail="Can I do it?" /><Metric label="Interest" value={assessment.interestScore} detail={`${assessment.baseInterestScore} baseline ${assessment.roleSpecificAdjustment ? `${assessment.roleSpecificAdjustment > 0 ? "+" : ""}${assessment.roleSpecificAdjustment} role` : "· no role adjustment"}`} /><Metric label="Direction" value={assessment.directionScore} detail="Where does it lead?" /><Metric label="Viability" value={assessment.viabilityScore} detail={assessment.ageLabel} /></div><div className="recommendation-summary"><Target /><div><strong>{recommendationLabel(assessment.recommendation)}</strong><p>{assessment.nextAction}</p></div></div></section>
        ...
        <section className="drawer-section focus-detail-card"><Target /><div><h3>{focusBucketLabel(assessment.focusBucket)}</h3><p>{assessment.focusReason}</p><small>Technical intensity {assessment.technicalIntensity}/100 · {assessment.technicalReason}</small></div></section>
