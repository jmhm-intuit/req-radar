...
function meaningfulWords(value: string): string[] {
  return value.toLowerCase().replace(/[^a-z0-9 ]/g, " ").split(/\s+/).filter((word) => word.length >= 4 && !["with", "that", "from", "this", "will", "your", "have", "into", "across", "through", "role"].includes(word));
}
...
function responsibilityCapability(text: string, assessment: JobAssessment) {
  return [...assessment.capabilitySkills].sort((left, right) => {
    const leftText = `${left.requirement.name} ${left.requirement.behavior} ${left.requirement.evidence}`;
    const rightText = `${right.requirement.name} ${right.requirement.behavior} ${right.requirement.evidence}`;
    return textOverlap(text, rightText) - textOverlap(text, leftText);
  })[0] || null;
}

function responsibilityInterest(text: string, assessment: JobAssessment) {
  return [...assessment.interestSignals].sort((left, right) => {
    const leftText = `${left.label} ${left.jobSignal.evidence.join(" ")}`;
    const rightText = `${right.label} ${right.jobSignal.evidence.join(" ")}`;
    const overlap = textOverlap(text, rightText) - textOverlap(text, leftText);
    return overlap || right.preference.importance - left.preference.importance;
  })[0] || null;
}

function interestLabel(value: number): string {
  if (value >= 75) return "Likely energizing";
  if (value >= 55) return "Potentially positive";
  if (value >= 40) return "Conditional";
  return "Potential drain";
}
  ...
  return <><button className="drawer-backdrop" onClick={onClose} aria-label="Close job details" /><aside className="drawer v2">
    ...
    <div className="drawer-body">
      {tab === "OVERVIEW" && <>
        ...
        <section className="drawer-section responsibility-fit-section"><div className="section-head"><div><h3>Responsibility-level fit</h3><p>Visualize the recurring work. Capability evidence and enjoyment are assessed separately.</p></div><Target /></div><div className="responsibility-fit-list">{assessment.successProfile.recurringResponsibilities.slice(0, 5).map((item) => { const capability = responsibilityCapability(item.statement, assessment); const interest = responsibilityInterest(item.statement, assessment); const interestScore = interest?.alignmentScore ?? assessment.fitSignature.attractionScore; return <article key={item.id}><header><div><span>Recurring responsibility</span><h4>{item.statement}</h4></div><small>{item.inferenceLevel.replace(/_/g, " ").toLowerCase()}</small></header><div className="responsibility-fit-grid"><div><span>Your evidence</span><strong className={capability ? skillClass(capability.status) : "skill-unknown"}>{capability ? MATCH_STATUS_LABELS[capability.status] : "Unknown"}</strong><p>{capability?.reason || "No directly linked candidate evidence yet."}</p>{capability?.evidence[0] && <blockquote>{capability.evidence[0]}</blockquote>}</div><div><span>Your interest</span><strong className={`interest-${fitTone(interestScore)}`}>{interestLabel(interestScore)} · {interestScore}</strong><p>{interest?.explanation || "Complete resume reflection and role discovery to make this interpretation more reliable."}</p></div></div>{(interestScore < 58 || !capability || capability.status === "UNKNOWN") && <footer><Lightbulb /><span>Question to resolve: {assessment.discovery.unresolvedQuestions[0] || `How much of the week is spent on this responsibility, and under what conditions?`}</span></footer>}</article>; })}</div></section>
