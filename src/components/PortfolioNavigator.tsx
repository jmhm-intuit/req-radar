    ...
    <div className="navigator-groups">{groups.length ? groups.map(([key, groupJobs]) => {
      ...
      return <section className={`panel navigator-group ${collapsed ? "collapsed" : ""}`} key={key}>
        ...
        {!collapsed && <>
          ...
          <div className="desktop-role-table"><JobTable jobs={shown} assessments={assessments} onOpen={onOpen} onUpdate={onUpdate} compact /></div>
          <div className="mobile-role-list">{shown.map((job) => { const assessment = assessments.get(job.id); return assessment ? <MobileRoleCard key={job.id} job={job} assessment={assessment} onOpen={onOpen} onUpdate={onUpdate} /> : null; })}</div>
