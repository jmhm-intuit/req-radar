...
export function workflowSignature(job: JobReq): string {
  return compactHash(stable({
    ...
    assessmentState: job.assessmentState,
    decisionState: job.decisionState,
    applicationStage: job.applicationStage,
    applicationNextAction: job.applicationNextAction,
    applicationNextActionDue: job.applicationNextActionDue,
    notes: job.notes,
