...
export function profileReadiness(profile: UserProfile): {
  ...
  const steps = [
    ...
    { label: "Resume experiences reflected", complete: (profile.resumeReflections || []).length >= 2 },
    ...
    return [{
      ...
      evidence: Array.isArray(raw.evidence) ? raw.evidence.flatMap((evidence) => {
        ...
        return [{
          ...
          ...(typeof entry.resumeReflectionId === "string" ? { resumeReflectionId: entry.resumeReflectionId } : {}),
...
export function normalizeLegacyProfile(value: unknown): UserProfile {
  ...
  const resumeReflections: ResumeAccomplishmentReflection[] = Array.isArray(profile.resumeReflections)
    ? (profile.resumeReflections as unknown[]).flatMap((item) => {
      if (!item || typeof item !== "object") return [];
      const raw = item as Partial<ResumeAccomplishmentReflection>;
      if (typeof raw.candidateId !== "string" || typeof raw.accomplishment !== "string") return [];
      return [{
        id: typeof raw.id === "string" ? raw.id : makeId("resume-reflection"),
        candidateId: raw.candidateId,
        accomplishment: raw.accomplishment,
        sourceLabel: typeof raw.sourceLabel === "string" ? raw.sourceLabel : "Resume",
        capabilityNames: Array.isArray(raw.capabilityNames) ? raw.capabilityNames.filter((value): value is string => typeof value === "string") : [],
        enjoyedActivities: Array.isArray(raw.enjoyedActivities) ? raw.enjoyedActivities.filter((value): value is ResumeAccomplishmentReflection["enjoyedActivities"][number] => typeof value === "string") : [],
        desiredFrequency: raw.desiredFrequency === "MAJOR" || raw.desiredFrequency === "RECURRING" || raw.desiredFrequency === "OCCASIONAL" || raw.desiredFrequency === "NECESSARY_ONLY" || raw.desiredFrequency === "NOT_IDEAL" ? raw.desiredFrequency : "RECURRING",
        energizingConditions: Array.isArray(raw.energizingConditions) ? raw.energizingConditions.filter((value): value is ResumeAccomplishmentReflection["energizingConditions"][number] => typeof value === "string") : [],
        drains: Array.isArray(raw.drains) ? raw.drains.filter((value): value is ResumeAccomplishmentReflection["drains"][number] => typeof value === "string") : [],
        confidence: raw.confidence === "DIRECT_EXPERIENCE" || raw.confidence === "RELATED_EXPERIENCE" || raw.confidence === "ESTIMATE" || raw.confidence === "UNSURE" ? raw.confidence : "DIRECT_EXPERIENCE",
