...
export function createJob(
  ...
  return {
    ...
    decisionState: status === "PURSUING" || status === "APPLIED" ? "PURSUE" : status === "NOT_PURSUING" || status === "CLOSED" ? "NOT_PURSUING" : status === "EXPLORING" || status === "MAYBE" ? "EXPLORE" : "INBOX",
    applicationStage: status === "APPLIED" ? "APPLIED" : status === "PURSUING" ? "VALIDATE_ROLE" : "NOT_STARTED",
    applicationNextAction: status === "PURSUING" ? "Validate the role and identify a networking contact" : "",
    applicationNextActionDue: "",
    applicationLastActivityAt: "",
    applicationNotes: "",
    skillOverrides: {},
    ageOverride: false,
    verifiedActiveAt: "",
